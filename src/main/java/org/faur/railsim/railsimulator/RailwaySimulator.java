...
import org.faur.railsim.serversynchronizer.SynchronizationServer;

/**
 * Railway Simulator engine. The mechanics for this engine are very simple.
 * First the simulator retrieve the train's schedule from controller and then start's listening
 * for any schedule modifications (there's a possibility that a vehicle got a delay. Therefore the 
 * controller needs to update the schedule and resolve conflicts. The simulator needs to be aware of them.)
 * <br><br>
 * Every time the simulator get's a signal from the synchronization server, it will send a command to the monitor
...
public class RailwaySimulator {
     ...
     * @param controllerAddress the controllerAddress to set
     */
	...
	posixOptions.addOption("controllerPort", true, "Controller's port binder.");
	...
	try {
		...
		this.setControllerPort(Integer.valueOf(commandLine.getOptionValue("controllerPort")));
	    }
	    if (commandLine.hasOption("controllerAddress")) {
		this.setControllerAddress(commandLine.getOptionValue("controllerAddress"));
	    }
	    if (commandLine.hasOption("synServerPort")) {
		this.setSyncServerPort(Integer.valueOf(commandLine.getOptionValue("syncServerPort")));
	    }
