...
/**
 * Server that listens for simulator clients.
 * If a connection is found, the manager will
 * start a command executor associated with the client.
 * 
 * @author faur
...
public class ConnectionManager extends Thread {
    ...
    public ConnectionManager(TrainManager manager, RailwayMonitorPanel monitorPanel, int port) {
    ...
    public void run() {
	try {
	    logger.log(Level.SEVERE, "Listening for connection on port " + String.valueOf(port));
	    server = new ServerSocket(port);
	    while (isAlive) {
		Socket client = server.accept();
		logger.log(Level.INFO, "A new connection accepted!");
		CommandExecutor cc = new CommandExecutor(tManager, monitorPanel);
		cc.setListener(client);
		cc.start();
		c.add(cc);
	    }
	} catch (IOException e) {
	    logger.log(Level.SEVERE, "Can't listen on port " + String.valueOf(port)  + "\n"+ e);
	}
