...
/**
 * Basic UI resource configuration like port where rail monitor should listen
 * for connection, railway map configuration file on the local disk and train
 * id's configuration file.
 * 
 * 
 * @author faur
...
public class ConfigurePanel extends JPanel implements ActionListener {
	...
	protected void init() {
		...
		layout.setHorizontalGroup(layout
				.createParallelGroup()
				.addGroup(
						layout.createSequentialGroup().addComponent(mapLabel)
				...
				.addGroup(
						layout.createSequentialGroup().addComponent(portLabel)
								.addComponent(tfPort).addComponent(btStart)
								.addComponent(btExit)).addComponent(txtPanel));

		layout.linkSize(SwingConstants.HORIZONTAL, btBrowseMap, btBrowseId);
		layout.linkSize(SwingConstants.VERTICAL, tfMapPath, tfIdPath, tfPort,
				btBrowseMap, btBrowseId, btStart, btExit);
		layout.setVerticalGroup(layout
				.createSequentialGroup()
				.addGroup(
						layout.createParallelGroup().addComponent(mapLabel)
								.addComponent(tfMapPath)
								.addComponent(btBrowseMap))
				.addGroup(
						layout.createParallelGroup().addComponent(idLabel)
								...
								.addComponent(btBrowseId))
				.addGroup(
						layout.createParallelGroup().addComponent(portLabel)
								.addComponent(tfPort).addComponent(btStart)
								.addComponent(btExit)).addComponent(txtPanel));
	}
		...
		if (bt.equals(btBrowseMap)) {
			...
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				...
				logger.log(Level.INFO,
						"Map path selected to " + file.getAbsolutePath());
			} else {
			...
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				...
				logger.log(Level.INFO,
						"Ids path selected to " + file.getAbsolutePath());
			} else {
				logger.log(Level.INFO, "Open command cancelled by user.\n");
			}
		}
	...
	public void setIdsPath(String idsPath) {
		...
		if ((tfIdPath.getText().endsWith(".csv"))
				&& (tfMapPath.getText().endsWith(".csv"))) {
