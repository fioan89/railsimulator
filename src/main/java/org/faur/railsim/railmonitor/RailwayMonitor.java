...
public class RailwayMonitor extends JFrame implements ActionListener {
	...
	int monitorWidth = (int) (7 * this.getWidth()) / 10;
	...
	layout.setHorizontalGroup(layout.createSequentialGroup()
		.addComponent(monitorPn, monitorWidth, monitorWidth, Short.MAX_VALUE)
		.addComponent(configurePn));

	layout.setVerticalGroup(layout.createParallelGroup()
		.addGroup(layout.createSequentialGroup()
			.addComponent(monitorPn))
			.addGroup(layout.createSequentialGroup()
				.addComponent(configurePn)));
	this.pack();
    }
	...
	if (bt.equals(configurePn.getBtStart())) {
	    ...
	    con = new ConnectionManager(tManager, monitorPn, configurePn.getPort());
	    con.start();
