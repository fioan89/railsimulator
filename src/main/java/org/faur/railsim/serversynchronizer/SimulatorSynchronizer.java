...
/**
 * Send a counter to every client in the connections list. In order to provide
 * speed improvements output streams are cached.
 * 
 * @author faur
...
public class SimulatorSynchronizer implements Runnable {
	...
	/**
	 * Constructs a synchronization server with the specified threshold and
	 * signaltime
	 * 
	 * @param connections
	 *            a synchronized list of clients.
	 * @param thresholding
	 *            a value after which the server counter is reset.
	 * @param signaltime
	 *            time between two synchronization messages.
	 */
	public SimulatorSynchronizer(List<Socket> connections, long threshold,
			long signaltime) {
		this.connections = connections;
		this.thresholding = threshold;
		this.isAlive = true;
		this.signaltime = signaltime;
		this.streams = new HashMap<Socket, PrintWriter>();
		this.logger = Logger.getLogger(SimulatorSynchronizer.class.getName());
	}
	...
	public void run() {
		...
		while (isAlive) {
			...
			logger.log(Level.INFO,
					"Sending synchronization message number {0}",
			...
			for (Socket client : connections) {
				...
				logger.log(Level.INFO, "Getting client");
				if (writer != null) {
					writer.print(String.valueOf(counter) + "\n");
					writer.flush();
					logger.log(Level.INFO, "counter sent");
				}
