...
public class SynchronizationServer {
	...
	public Options constructPosixOptions() {
		...
		posixOptions.addOption("port", true,
				"Set listening port for client connections.");
		posixOptions.addOption("threshold", true,
				"Set the threshold after which the server counter is reset.");
		posixOptions
				.addOption(
						"signaltime",
						true,
						"Set the time in milliseconds after which the server counter is increased and the message is sent.");
	...
	public void usePosixParser(final String[] commandLineArguments) {
		...
		try {
			...
			if (commandLine.getArgs().length < 1) {
				help.printHelp(SynchronizationServer.class.getName(),
						posixOptions, true);
			}
			if (commandLine.hasOption("port")) {
				this.setListeningPort(Integer.parseInt(commandLine
						.getOptionValue("port")));
			}
			if (commandLine.hasOption("threshold")) {
				this.setThreshold(Integer.parseInt(commandLine
						.getOptionValue("threshold")));
			}
			if (commandLine.hasOption("signaltime")) {
				this.setTimeBeforeNextMsg(Integer.parseInt(commandLine
						.getOptionValue("signaltime")));
			}
	...
	public void startSynchronization() {
		SimulatorListener simulatorListener = new SimulatorListener(
				connections, getListeningPort());
		Thread tListener = new Thread(simulatorListener);
		tListener.start();

		SimulatorSynchronizer simulatorSynchronizer = new SimulatorSynchronizer(
				connections, threshold, timeBeforeNextMsg);
